    ...
    <div class="container mx-auto px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div class="grid gap-6 lg:grid-cols-3">
        ...
        <div class="space-y-3 lg:col-span-1">
          <div v-for="message in messages" :key="message.id" @click="selectedMessage = message.id" :class="[
            ...
            <div class="flex items-start gap-3">
              <div class="relative">
                <img :src="message.clientImage" :alt="message.clientName" class="h-12 w-12 rounded-full object-cover" />
          ...
          <div v-if="selectedMessage" class="rounded-lg border border-gray-200 bg-white">
            ...
            <div class="border-b border-gray-200 p-4">
              <div class="flex items-center gap-3">
                <img :src="messages.find(m => m.id === selectedMessage)?.clientImage"
                  :alt="messages.find(m => m.id === selectedMessage)?.clientName"
                  class="h-10 w-10 rounded-full object-cover" />
                <div>
                  <h3 class="font-semibold text-gray-900">{{messages.find(m => m.id === selectedMessage)?.clientName}}
                  </h3>
              ...
              <div class="space-y-4">
                ...
                <div class="flex justify-start">
                  <div class="max-w-[80%] rounded-lg bg-gray-100 px-4 py-2">
                    ...
                    <p class="mt-1 text-xs text-gray-500">{{formatTime(messages.find(m => m.id ===
                      selectedMessage)?.timestamp || '')}}</p>
                  </div>
                </div>
                ...
                <div class="flex justify-end">
                  <div class="max-w-[80%] rounded-lg bg-purple-600 px-4 py-2">
                    <p class="text-sm text-white">Hola! Claro, con gusto. ¿Qué dimensiones necesitas y en qué zona estás
                      ubicado?</p>
            ...
            <div class="border-t border-gray-200 p-4">
              <form @submit.prevent="sendMessage" class="flex gap-2">
                <input v-model="messageText" type="text" placeholder="Escribe tu mensaje..."
